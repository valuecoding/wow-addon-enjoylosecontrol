...
if not InterfaceOptions_AddCategory then
	local queuedOptionPanels = {}
	local queuedOptionsFrame
	...
	local function RegisterOptionsPanel(panel)
		...
		if Settings and Settings.RegisterCanvasLayoutCategory and Settings.RegisterAddOnCategory then
			panel.OnCommit = panel.OnCommit or panel.okay
			panel.OnDefault = panel.OnDefault or panel.default
			panel.OnRefresh = panel.OnRefresh or panel.refresh
			if panel.parent and Settings.GetCategory and Settings.RegisterCanvasLayoutSubcategory then
				local parentCategory = Settings.GetCategory(panel.parent)
				if not parentCategory then
					return false
				end
				local subCategory = Settings.RegisterCanvasLayoutSubcategory(parentCategory, panel, panel.name, panel.name)
	...
	function InterfaceOptions_AddCategory(panel)
		if RegisterOptionsPanel(panel) then
			return
		end
		table.insert(queuedOptionPanels, panel)
		if queuedOptionsFrame then
			return
		end
		queuedOptionsFrame = CreateFrame("Frame")
		queuedOptionsFrame:RegisterEvent("ADDON_LOADED")
		queuedOptionsFrame:RegisterEvent("PLAYER_LOGIN")
		queuedOptionsFrame:SetScript("OnEvent", function(self)
			for i = #queuedOptionPanels, 1, -1 do
				if RegisterOptionsPanel(queuedOptionPanels[i]) then
					table.remove(queuedOptionPanels, i)
				end
			end
			if #queuedOptionPanels == 0 then
...
if not BlizzardOptionsPanel_CheckButton_Enable then
	function BlizzardOptionsPanel_CheckButton_Enable(checkButton)
		if not checkButton then return end
		checkButton:Enable()
		if checkButton.Text and checkButton.Text.SetAlpha then
			checkButton.Text:SetAlpha(1)
		end
		if checkButton.SetAlpha then
...
if not BlizzardOptionsPanel_CheckButton_Disable then
	function BlizzardOptionsPanel_CheckButton_Disable(checkButton)
		if not checkButton then return end
		checkButton:Disable()
		if checkButton.Text and checkButton.Text.SetAlpha then
			checkButton.Text:SetAlpha(0.5)
		end
		if checkButton.SetAlpha then
...
local SetSliderVisualAlpha
local UpdateSliderVisual

if not BlizzardOptionsPanel_Slider_Enable then
	function BlizzardOptionsPanel_Slider_Enable(slider)
		if not slider then return end
		slider:Enable()
		SetSliderVisualAlpha(slider, 1)
		if slider.Text and slider.Text.SetAlpha then
			slider.Text:SetAlpha(1)
		end
		if slider.Low and slider.Low.SetAlpha then
			slider.Low:SetAlpha(1)
		end
		if slider.High and slider.High.SetAlpha then
			slider.High:SetAlpha(1)
		end
	end
end

if not BlizzardOptionsPanel_Slider_Disable then
	function BlizzardOptionsPanel_Slider_Disable(slider)
		if not slider then return end
		slider:Disable()
		SetSliderVisualAlpha(slider, 0.5)
		if slider.Text and slider.Text.SetAlpha then
			slider.Text:SetAlpha(0.5)
		end
		if slider.Low and slider.Low.SetAlpha then
			slider.Low:SetAlpha(0.5)
		end
		if slider.High and slider.High.SetAlpha then
			slider.High:SetAlpha(0.5)
		end
	end
end

SetSliderVisualAlpha = function(slider, alpha)
	if not slider then
		return
	end
	...
	local thumbTexture = slider.GetThumbTexture and slider:GetThumbTexture() or nil
	if thumbTexture then
		if thumbTexture.SetAlpha then
			thumbTexture:SetAlpha(0)
		end
		if thumbTexture.SetVertexColor then
			thumbTexture:SetVertexColor(1, 1, 1, 0)
		end
	end
	local i = 1
	local region = select(i, slider:GetRegions())
	while region do
		if slider._lcBaseTextures and slider._lcBaseTextures[region] then
			...
			if region.GetObjectType and region:GetObjectType() == "Texture" and region.SetVertexColor then
				region:SetVertexColor(1, 1, 1, alpha)
			end
		end
		i = i + 1
		region = select(i, slider:GetRegions())
	end
	if slider._lcTrackBg then
		slider._lcTrackBg:SetAlpha(alpha)
	end
	if slider._lcTrackFill then
		slider._lcTrackFill:SetAlpha(alpha)
	end
	if slider._lcThumb then
		slider._lcThumb:SetAlpha(alpha)
	end
end

UpdateSliderVisual = function(slider)
	if not slider or not slider._lcTrackBg then
		return
	end
	local minValue, maxValue = slider:GetMinMaxValues()
	local currentValue = slider:GetValue() or minValue or 0
	...
	local trackWidth = slider._lcTrackBg:GetWidth() or 0
	local fillWidth = math.max(8, trackWidth * percent)
	slider._lcTrackFill:SetWidth(fillWidth)
	slider._lcThumb:ClearAllPoints()
	slider._lcThumb:SetPoint("CENTER", slider._lcTrackBg, "LEFT", trackWidth * percent, 0)
end
...
local function ResolveAnchorReference(anchorValue)
	if type(anchorValue) == "table" and not anchorValue.GetObjectType then
		for _, candidate in ipairs(anchorValue) do
			local resolved = ResolveAnchorReference(candidate)
	...
	if type(anchorValue) == "string" then
		return _G[anchorValue] or _GF(anchorValue)
	end
	return anchorValue
end
...
local anchors = {
	...
	Blizzard = {
		...
		arena1       = { "ArenaEnemyFrame1ClassPortrait", "ArenaEnemyFrame1.ClassPortrait", "ArenaEnemyFrame1.Portrait", "ArenaEnemyMatchFrame1ClassPortrait", "ArenaEnemyMatchFrame1.ClassPortrait", "ArenaEnemyMatchFrame1.Portrait", "ArenaEnemyFrame1", "ArenaEnemyMatchFrame1" },
		arena2       = { "ArenaEnemyFrame2ClassPortrait", "ArenaEnemyFrame2.ClassPortrait", "ArenaEnemyFrame2.Portrait", "ArenaEnemyMatchFrame2ClassPortrait", "ArenaEnemyMatchFrame2.ClassPortrait", "ArenaEnemyMatchFrame2.Portrait", "ArenaEnemyFrame2", "ArenaEnemyMatchFrame2" },
		arena3       = { "ArenaEnemyFrame3ClassPortrait", "ArenaEnemyFrame3.ClassPortrait", "ArenaEnemyFrame3.Portrait", "ArenaEnemyMatchFrame3ClassPortrait", "ArenaEnemyMatchFrame3.ClassPortrait", "ArenaEnemyMatchFrame3.Portrait", "ArenaEnemyFrame3", "ArenaEnemyMatchFrame3" },
		arena4       = { "ArenaEnemyFrame4ClassPortrait", "ArenaEnemyFrame4.ClassPortrait", "ArenaEnemyFrame4.Portrait", "ArenaEnemyMatchFrame4ClassPortrait", "ArenaEnemyMatchFrame4.ClassPortrait", "ArenaEnemyMatchFrame4.Portrait", "ArenaEnemyFrame4", "ArenaEnemyMatchFrame4" },
		arena5       = { "ArenaEnemyFrame5ClassPortrait", "ArenaEnemyFrame5.ClassPortrait", "ArenaEnemyFrame5.Portrait", "ArenaEnemyMatchFrame5ClassPortrait", "ArenaEnemyMatchFrame5.ClassPortrait", "ArenaEnemyMatchFrame5.Portrait", "ArenaEnemyFrame5", "ArenaEnemyMatchFrame5" }
	},
	BlizzardRaidFrames = {
...
function LoseControl:CheckAnchor(forceCheck)
	...
	if ((self.frame.anchor ~= "None") and (forceCheck or self.anchor == UIParent or self.anchor == nil)) then
		...
		if anchorObj ~= nil then
			...
			local newAnchor = ResolveAnchorReference(anchorObj[self.fakeUnitId or self.unitId])
...
function LoseControl:PLAYER_ENTERING_WORLD() -- this correctly anchors enemy arena frames that aren't created until you zone into an arena
	...
	self.anchor = ResolveAnchorReference(anchors[frame.anchor] and anchors[frame.anchor][self.fakeUnitId or unitId]) or UIParent
...
function LoseControl:CheckStatusPartyFrameChange(value)
	...
	if (value ~= self.useCompactPartyFrames) then
		...
		if (frame.anchor == "Blizzard") or (unitId == "partyplayer") then
			...
			self.anchor = ResolveAnchorReference(anchors[frame.anchor] and anchors[frame.anchor][unitId]) or UIParent
			self.parent:SetParent(self.anchor:GetParent() or UIParent or nil)
			self.defaultFrameStrata = self:GetFrameStrata()
			self:GetParent():ClearAllPoints()
			self:GetParent():SetPoint(
...
function LoseControl:StopMoving()
	...
	self.anchor = ResolveAnchorReference(anchors[frame.anchor] and anchors[frame.anchor][self.fakeUnitId or self.unitId]) or UIParent
	self.parent:SetParent(self.anchor:GetParent() or UIParent or nil)
	self.defaultFrameStrata = self:GetFrameStrata()
	self:ClearAllPoints()
	self:GetParent():ClearAllPoints()
...
local function CreateCopyEditBox(parent, label, text, offsetY)
	local labelText = parent:CreateFontString(nil, "ARTWORK", "GameFontNormal")
	labelText:SetPoint("TOPLEFT", 18, offsetY)
	labelText:SetText(label)

	local editbox = CreateEditBox(label .. "CopyBox", parent, 500, 255, parent:GetName() .. label:gsub("%s+", "") .. "CopyBox")
	editbox:SetPoint("TOPLEFT", labelText, "BOTTOMLEFT", 0, -8)
	...
	local copyButton = CreateFrame("Button", parent:GetName() .. label:gsub("%s+", "") .. "CopyButton", parent, "UIPanelButtonTemplate")
	copyButton:SetSize(78, 22)
	copyButton:SetPoint("LEFT", editbox, "RIGHT", 10, 0)
	copyButton:SetText("Select")
	...
	editbox:SetScript("OnTextChanged", function(self, userInput)
		if userInput and self:GetText() ~= self.originalText then
			self:SetText(self.originalText)
			self:SetCursorPosition(0)
			self:HighlightText()
		end
	...
	return labelText, editbox, copyButton
end
...
local function CreateSlider(text, parent, low, high, step, width, createBox, globalName)
	...
	slider._lcBaseTextures = {}
	local regionIndex = 1
	local baseRegion = select(regionIndex, slider:GetRegions())
	while baseRegion do
		if baseRegion.GetObjectType and baseRegion:GetObjectType() == "Texture" then
			slider._lcBaseTextures[baseRegion] = true
			if baseRegion.SetAlpha then
				baseRegion:SetAlpha(0)
			end
			if baseRegion.SetVertexColor then
				baseRegion:SetVertexColor(1, 1, 1, 0)
			end
		end
		regionIndex = regionIndex + 1
		baseRegion = select(regionIndex, slider:GetRegions())
	end
	local defaultThumb = slider.GetThumbTexture and slider:GetThumbTexture() or nil
	if defaultThumb then
		defaultThumb:SetAlpha(0)
	end
	slider._lcTrackBg = slider:CreateTexture(nil, "BACKGROUND")
	slider._lcTrackBg:SetColorTexture(0.12, 0.12, 0.12, 0.95)
	slider._lcTrackBg:SetPoint("TOPLEFT", slider, "TOPLEFT", 16, -17)
	slider._lcTrackBg:SetPoint("TOPRIGHT", slider, "TOPRIGHT", -16, -17)
	slider._lcTrackBg:SetHeight(8)
	slider._lcTrackFill = slider:CreateTexture(nil, "ARTWORK")
	slider._lcTrackFill:SetColorTexture(0.95, 0.78, 0.12, 0.95)
	slider._lcTrackFill:SetPoint("LEFT", slider._lcTrackBg, "LEFT", 0, 0)
	slider._lcTrackFill:SetHeight(8)
	slider._lcTrackFill:SetWidth(8)
	slider._lcThumb = slider:CreateTexture(nil, "OVERLAY")
	slider._lcThumb:SetColorTexture(1, 0.92, 0.45, 1)
	slider._lcThumb:SetSize(10, 18)
	SetSliderVisualAlpha(slider, 1)
...
local O = "LoseControlOptionsPanel"
...
local function OpenOptionsPanel(panel)
	...
	if Settings and Settings.OpenToCategory then
		local category = Settings.GetCategory and Settings.GetCategory(panel.name or addonName) or nil
		if category and pcall(Settings.OpenToCategory, category.ID or category) then
			return
		end
		if pcall(Settings.OpenToCategory, panel.name or addonName) then
			return
		end
	end
...
local Unlock = CreateFrame("CheckButton", O.."Unlock", OptionsPanel.container, "InterfaceOptionsCheckButtonTemplate")
...
local DisableBlizzardCooldownCount = CreateFrame("CheckButton", O.."DisableBlizzardCooldownCount", OptionsPanel.container, "InterfaceOptionsCheckButtonTemplate")
...
local DisableCooldownCount = CreateFrame("CheckButton", O.."DisableCooldownCount", OptionsPanel.container, "InterfaceOptionsCheckButtonTemplate")
...
for _, v in ipairs({ "player", "pet", "target", "targettarget", "focus", "focustarget", "party", "arena", "raid", "nameplate" }) do
	...
	local DisableInBG
	if v == "party" then
		DisableInBG = CreateFrame("CheckButton", O..v.."DisableInBG", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "party" then
		DisableInArena = CreateFrame("CheckButton", O..v.."DisableInArena", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	local DisableInRaid
	if v == "party" then
		DisableInRaid = CreateFrame("CheckButton", O..v.."DisableInRaid", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "target" or v == "focus" or v == "targettarget" or v == "focustarget" or v == "nameplate" then
		ShowNPCInterrupts = CreateFrame("CheckButton", O..v.."ShowNPCInterrupts", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "targettarget" or v == "focustarget" then
		DisablePlayerTargetTarget = CreateFrame("CheckButton", O..v.."DisablePlayerTargetTarget", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "targettarget" then
		DisableTargetTargetTarget = CreateFrame("CheckButton", O..v.."DisableTargetTargetTarget", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "targettarget" then
		DisablePlayerTargetPlayerTargetTarget = CreateFrame("CheckButton", O..v.."DisablePlayerTargetPlayerTargetTarget", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "targettarget" then
		DisableTargetDeadTargetTarget = CreateFrame("CheckButton", O..v.."DisableTargetDeadTargetTarget", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "focustarget" then
		DisableFocusFocusTarget = CreateFrame("CheckButton", O..v.."DisableFocusFocusTarget", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "focustarget" then
		DisablePlayerFocusPlayerFocusTarget = CreateFrame("CheckButton", O..v.."DisablePlayerFocusPlayerFocusTarget", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "focustarget" then
		DisableFocusDeadFocusTarget = CreateFrame("CheckButton", O..v.."DisableFocusDeadFocusTarget", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	local UseSpellInsteadSchoolMiniIcon = CreateFrame("CheckButton", OptionsPanelFrame:GetName().."UseSpellInsteadSchoolMiniIcon", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v ~= "arena" then
		local FriendlyInterrupt = CreateFrame("CheckButton", O..v.."FriendlyInterrupt", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "target" or v == "targettarget" or v == "focus" or v == "focustarget" or v == "arena" or v == "nameplate" then
		local EnemyInterrupt = CreateFrame("CheckButton", O..v.."EnemyInterrupt", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	for _, cat in pairs(catListEnChecksButtons) do
		if v ~= "arena" then
			local FriendlyBuff = CreateFrame("CheckButton", O..v.."Friendly"..cat.."Buff", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
		...
		if v ~= "arena" then
			local FriendlyDebuff = CreateFrame("CheckButton", O..v.."Friendly"..cat.."Debuff", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
		...
		if v == "target" or v == "targettarget" or v == "focus" or v == "focustarget" or v == "arena" or v == "nameplate" then
			local EnemyBuff = CreateFrame("CheckButton", O..v.."Enemy"..cat.."Buff", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
		...
		if v == "target" or v == "targettarget" or v == "focus" or v == "focustarget" or v == "arena" or v == "nameplate" then
			local EnemyDebuff = CreateFrame("CheckButton", O..v.."Enemy"..cat.."Debuff", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if (v == "player") then
		...
		local FriendlyInterruptPlayer2 = CreateFrame("CheckButton", O..v.."FriendlyInterruptPlayer2", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
		...
		for _, cat in pairs(catListEnChecksButtons) do
			local FriendlyBuffPlayer2 = CreateFrame("CheckButton", O..v.."Friendly"..cat.."BuffPlayer2", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
			...
			local FriendlyDebuffPlayer2 = CreateFrame("CheckButton", O..v.."Friendly"..cat.."DebuffPlayer2", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "player" then
		DuplicatePlayerPortrait = CreateFrame("CheckButton", O..v.."DuplicatePlayerPortrait", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	if v == "party" then
		EnabledPartyPlayerIcon = CreateFrame("CheckButton", O..v.."EnabledPartyPlayerIcon", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
	...
	local Enabled = CreateFrame("CheckButton", O..v.."Enabled", OptionsPanelFrame.container, "InterfaceOptionsCheckButtonTemplate")
...
-------------------------------------------------------------------------------
-- Info sub-option frame
do
	local supportLink = "https://streamelements.com/soloqgpt-4747d/tip"
	local curseForgeLink = "https://www.curseforge.com/members/enjoymygripz/projects"
	local addonVersion = GetAddOnMetadata(addonName, "Version") or "1.0.0"

	local InfoPanel = CreateFrame("Frame", O.."Info")
	InfoPanel.parent = addonName
	InfoPanel.name = "Info"

	InfoPanel.scrollframe = InfoPanel.scrollframe or CreateFrame("ScrollFrame", InfoPanel:GetName().."ScrollFrame", InfoPanel, "UIPanelScrollFrameTemplate")
	InfoPanel.scrollchild = InfoPanel.scrollchild or CreateFrame("Frame", InfoPanel:GetName().."ScrollChild")

	InfoPanel.scrollbar = _G[InfoPanel.scrollframe:GetName().."ScrollBar"]
	InfoPanel.scrollupbutton = _G[InfoPanel.scrollframe:GetName().."ScrollBarScrollUpButton"]
	InfoPanel.scrolldownbutton = _G[InfoPanel.scrollframe:GetName().."ScrollBarScrollDownButton"]
	InfoPanel.scrollupbutton:ClearAllPoints()
	InfoPanel.scrollupbutton:SetPoint("TOPRIGHT", InfoPanel.scrollframe, "TOPRIGHT", -2, -2)
	InfoPanel.scrolldownbutton:ClearAllPoints()
	InfoPanel.scrolldownbutton:SetPoint("BOTTOMRIGHT", InfoPanel.scrollframe, "BOTTOMRIGHT", -2, 2)

	InfoPanel.scrollbar:ClearAllPoints()
	InfoPanel.scrollbar:SetPoint("TOP", InfoPanel.scrollupbutton, "BOTTOM", 0, -2)
	InfoPanel.scrollbar:SetPoint("BOTTOM", InfoPanel.scrolldownbutton, "TOP", 0, 2)

	InfoPanel.scrollframe:SetScrollChild(InfoPanel.scrollchild)
	InfoPanel.scrollframe:SetAllPoints(InfoPanel)
	InfoPanel.scrollchild:SetSize(623, 560)

	InfoPanel.container = InfoPanel.container or CreateFrame("Frame", InfoPanel.scrollchild:GetName().."Container", InfoPanel.scrollchild)
	InfoPanel.container:SetAllPoints(InfoPanel.scrollchild)

	local title = InfoPanel.container:CreateFontString(nil, "ARTWORK", "GameFontNormalLarge")
	title:SetPoint("TOPLEFT", 16, -16)
	title:SetText("EnjoyLoseControl")

	local versionText = InfoPanel.container:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall")
	versionText:SetPoint("TOPLEFT", title, "BOTTOMLEFT", 0, -6)
	versionText:SetText("|cffc0c0c0Version " .. addonVersion .. "|r")
	versionText:SetShadowOffset(1, -1)
	versionText:SetShadowColor(0, 0, 0, 0.75)

	local body = InfoPanel.container:CreateFontString(nil, "ARTWORK", "GameFontHighlight")
	body:SetPoint("TOPLEFT", versionText, "BOTTOMLEFT", 0, -16)
	body:SetWidth(560)
	body:SetJustifyH("LEFT")
	body:SetJustifyV("TOP")
	body:SetText("EnjoyLoseControl is a TBC Anniversary fork maintained by |cffffd100enjoymygripz|r\n\nIf you find a bug, a Lua error, or have an addon idea, feedback, or any questions, please reach out on CurseForge. Private messages are also welcome. When reporting issues, include the Lua error text and a short description of what happened.")

	local supportLabel, supportBox = CreateCopyEditBox(InfoPanel.container, "Support / Tip Link", supportLink, -142)
	local curseForgeLabel, curseForgeBox = CreateCopyEditBox(InfoPanel.container, "CurseForge", curseForgeLink, -212)

	local footer = InfoPanel.container:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall")
	footer:SetPoint("TOPLEFT", curseForgeBox, "BOTTOMLEFT", 0, -18)
	footer:SetWidth(560)
	footer:SetJustifyH("LEFT")
	footer:SetJustifyV("TOP")
	footer:SetText("Click Select, then press Ctrl+C to copy the link.")

	InfoPanel.default = function()
	end

	InfoPanel.refresh = function()
		supportBox.originalText = supportLink
		curseForgeBox.originalText = curseForgeLink
		supportBox:SetText(supportLink)
		supportBox:SetCursorPosition(0)
		supportBox:HighlightText(0, 0)
		curseForgeBox:SetText(curseForgeLink)
		curseForgeBox:SetCursorPosition(0)
		curseForgeBox:HighlightText(0, 0)
	end

	InterfaceOptions_AddCategory(InfoPanel)
end

-------------------------------------------------------------------------------
SLASH_ENJOYLOSECONTROL1 = "/elc"
SLASH_ENJOYLOSECONTROL2 = "/enjoylosecontrol"
SLASH_ENJOYLOSECONTROL3 = "/lc"
SLASH_ENJOYLOSECONTROL4 = "/losecontrol"
...
SlashCmdList["ENJOYLOSECONTROL"] = function(cmd)
	local args = {}
	...
	if SlashCmd[args[1]] then
		...
		print(addonName, ": Type \"/elc help\" for more options.")
		OpenOptionsPanel(OptionsPanel)
	end
end
